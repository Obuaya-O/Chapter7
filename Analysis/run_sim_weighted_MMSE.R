#!/usr/bin/env Rscript
# PURPOSE: Executes the Monte Carlo simulation to evaluate the Weighted Biomarker Panel using MMSE as the primary clinical endpoint 
...
print(paste("Weighted MMSE Scen:", job_id, "| N:", scen$sample_size))
...
for (i in 1:reps_per_chunk) {
  ...
  if (scen$missing_type == "MCAR") sim_d <- sim_d %>% filter(time == 0 | runif(n()) > 0.15)
    ...
    # Fast decliners (Based on ADAS slope since it drives underlying severity)
  ...
  # MODEL FITS 
  fit_clin  <- tryCatch(lmer(MMSE ~ time * group + (1 | sub_id), data = sim_d), error=function(e) NULL)
...
dir.create("results_weighted_MMSE", showWarnings=F)
saveRDS(results, paste0("results_weighted_MMSE/job_", job_id, "_chunk_", chunk_id, ".rds"))